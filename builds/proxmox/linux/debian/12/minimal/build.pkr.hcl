// Packer build file for Debian 12 minimal variants
packer {
  required_plugins {
    proxmox = {
      version = ">= 1.1.3"
...
// Build block for all minimal variants
build {
  name = "debian_12_minimal"
  ...
  provisioner "ansible" {
    playbook_file = "${path.root}/../../ansible/minimal-${source.name}.yml"
    use_proxy     = false
    user          = var.username
  }
